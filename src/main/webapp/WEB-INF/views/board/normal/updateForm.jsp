      ...
      <div class="container">
        <div class="row">
		  ...
		  <div class="col-lg-6" style="text-align: center; margin: 0 auto">
            <form action="${pageContext.request.contextPath}/board/normal/update" method="post" enctype="multipart/form-data" name="registerForm" >
              <div class="form-group">
                <input type="hidden" name="boardNo" value="${normal.boardNo}">
				제목 : 
				<input type="text" class="form-control" name="title" value="${normal.title}"> 
				<p>
              </div>
              <div class="form-group mt-3">
                <select class="form-control" name="normalCategoryNo" style="font-size:14px; color:gray;">
				  ...
				  <option value="3" <c:if test="${normal.normalCategory.normalCategoryNo == 3}">selected</c:if>>Q&A</option>
				  <option value="4" <c:if test="${normal.normalCategory.normalCategoryNo == 4}">selected</c:if>>정보</option>
				  <option value="5" <c:if test="${normal.normalCategory.normalCategoryNo == 5}">selected</c:if>>예판/핫딜</option>
                </select>
              </div>
              <div class="form-group mt-3">
                내용 :<p>
				<textarea class="form-control" rows="15" name="content">${normal.content}</textarea>
				<p>
              </div>
             <div class="form-group mt-3">
                <input class="form-control" type="file" name="filename" multiple style="font-size:14px; color:gray;">
              </div>
              <input type="submit" class="text-center mt-3" value="수정하기">              
             <input type="button" class="text-center mt-3" value="돌아가기" onclick="history.back()">
            </form>
          </div>
        </div>
